...
function PlayerBar({
  ...
  onPlayPause, onNext, onPrev, onSeek, onVolumeChange, onToggleLike
}) {
  const [isDraggingSeekBar, setIsDraggingSeekBar] = useState(false);
  const [isMuted, setIsMuted] = useState(volume === 0);
  const [lastVolumeBeforeMute, setLastVolumeBeforeMute] = useState(volume > 0 ? volume : 0.5);

  useEffect(() => {
    setIsMuted(volume === 0);
    if (volume > 0 && !isMuted) { // Solo actualizar lastVolume si no estamos muteando activamente
      setLastVolumeBeforeMute(volume);
    }
  }, [volume, isMuted]); // Depender también de isMuted evita un loop si se mutea y luego se cambia el volumen

  const handleSeekBarChange = (e) => onSeek(parseFloat(e.target.value));
  const handleVolumeSliderChange = (e) => onVolumeChange(parseFloat(e.target.value));
  ...
  if (!song) {
    return <div className="player-bar placeholder"><span>Selecciona una canción</span></div>;
  }
  ...
  return (
    <div className="player-bar">
      <div className="player-song-info-wrapper">
        <img src={song.albumArtUrl || song.albumArt || 'https://via.placeholder.com/60?text=Art'} alt={song.title || "Portada"} className="player-album-art" />
        ...
        {onToggleLike && ( 
            <button onClick={() => onToggleLike(song.id, song.userHasLiked, song.likeCount)} className={`control-button like-button-player-bar ${song.userHasLiked ? 'liked' : ''}`} title={song.userHasLiked ? "Quitar Me gusta" : "Me gusta"}>
      ...
      <div className="player-controls-progress-wrapper">
        <div className="player-main-controls">
          <button onClick={onPrev} className="control-button prev-button" title="Anterior" disabled={!onPrev}><PrevIconPlayerBar /></button>
          <button onClick={onPlayPause} className="control-button play-pause-main-button" title={isPlaying ? "Pausar" : "Reproducir"} disabled={!onPlayPause}>
            {isPlaying ? <PauseIconPlayerBar /> : <PlayIconPlayerBar />}
          </button>
          <button onClick={onNext} className="control-button next-button" title="Siguiente" disabled={!onNext}><NextIconPlayerBar /></button>
        </div>
        <div className="player-progress-bar-section">
          <span className="time current-time">{formatTime(currentTime)}</span>
          <div className="progress-bar-container">
            <input type="range" min="0" max={duration || 0} value={currentTime} onChange={handleSeekBarChange} onMouseDown={() => setIsDraggingSeekBar(true)} onMouseUp={() => setIsDraggingSeekBar(false)} className="seek-bar" aria-label="Barra de progreso" style={{ backgroundSize: `${isDraggingSeekBar || currentTime > 0 ? progressPercent : 0}% 100%` }} disabled={!duration}/>
          </div>
          <span className="time total-time">{formatTime(duration)}</span>
        </div>
      </div>
      <div className="player-volume-controls-wrapper">
        <button onClick={toggleMute} className="control-button volume-mute-button" title={isMuted ? "Quitar silencio" : "Silenciar"}>
            {isMuted || volume === 0 ? <VolumeMuteIconPlayerBar /> : <VolumeUpIconPlayerBar />}
        </button>
        <div className="volume-bar-container">
            <input type="range" min="0" max="1" step="0.01" value={isMuted ? 0 : volume} onChange={handleVolumeSliderChange} className="volume-bar" aria-label="Control de volumen" style={{ backgroundSize: `${(isMuted ? 0 : volume) * 100}% 100%` }}/>
        </div>
      </div>
    </div>
  );
